...
func InstallLinux() flow.Flow {
	return flow.Flow{
		...
		Steps: []flow.Step{
			...
			newStep("init temp", initTemp, nil),
			newStep("extract", extractAndUpdateVersion, nil),
			newStep("configure tor", installTor, nil),
			newStep("configure dapp", configureDapp, nil),
			newStep("install container", installContainer, nil),
			newStep("enable and start container", enableAndStartContainer, nil),
			newStep("create database", createDatabase, nil),
			newStep("install products", installProducts, nil),
			newStep("disable container if client", disablContainerIfClient, nil),
			newStep("install supervisor", installSupervisorIfClient, nil),
			newStep("check container running", finalize, nil),
			newStep("stop container if client", stopContainerIfClient, nil),
			newStep("remove temp", removeTemp, nil),
		},
	}
}
